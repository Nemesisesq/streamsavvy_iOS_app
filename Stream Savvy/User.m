...
+ (void)logingWithFacebook:(NSString *)token success: (void (^)(void))successBlock failure:(void (^)(void))failBlock {
    NSString *url  = @"http://ss-master-staging.herokuapp.com/social/facebook";
    [[AFAPIClient  sharedClient:nil] GET:url parameters:nil
                                 success:^(NSURLSessionDataTask *task, id json) {
                                     NSLog(@"1 - %@", ((NSDictionary *)json));
                                     [UserPrefs setToken: [((NSDictionary *)json)
                                                           objectForKey:@"token"]];
                                     successBlock();
                                     
                                 } failure:^(NSURLSessionDataTask *task, NSError *error) {
                                     NSLog(@"~~~>%@", error);
                                     [Constants showAlert:@"Whoops!" withMessage:@"Something went wrong with social login"];
                                     failBlock();
                                 }];
}
